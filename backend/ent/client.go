...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.RedeemCode, c.Setting, c.SubscriptionOrder, c.UsageCleanupTask, c.UsageLog,
...
// QuerySubscription queries the subscription edge of a SubscriptionOrder.
func (c *SubscriptionOrderClient) QuerySubscription(_m *SubscriptionOrder) *UserSubscriptionQuery {
	query := (&UserSubscriptionClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
func (c *UserSubscriptionClient) QueryOrders(_m *UserSubscription) *SubscriptionOrderQuery {
	query := (&SubscriptionOrderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		...
		Setting, SubscriptionOrder, UsageCleanupTask, UsageLog, User, UserAllowedGroup,
