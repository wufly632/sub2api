...
type UserSubscriptionQuery struct {
	...
	withOrders         *SubscriptionOrderQuery
...
func (_q *UserSubscriptionQuery) QueryOrders() *SubscriptionOrderQuery {
...
func (_q *UserSubscriptionQuery) WithOrders(opts ...func(*SubscriptionOrderQuery)) *UserSubscriptionQuery {
	query := (&SubscriptionOrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserSubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserSubscription, error) {
	...
	if query := _q.withOrders; query != nil {
		if err := _q.loadOrders(ctx, query, nodes,
			func(n *UserSubscription) { n.Edges.Orders = []*SubscriptionOrder{} },
			func(n *UserSubscription, e *SubscriptionOrder) { n.Edges.Orders = append(n.Edges.Orders, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserSubscriptionQuery) loadOrders(ctx context.Context, query *SubscriptionOrderQuery, nodes []*UserSubscription, init func(*UserSubscription), assign func(*UserSubscription, *SubscriptionOrder)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionorder.FieldSubscriptionID)
	}
